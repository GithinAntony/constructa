        ...
        if request.path.startswith('/clients/'):
            ...
            for key, value in required_session_vars.items():
                ...
                # Check if the current URL is the login page
                    if not request.path.startswith(reverse('clients:sign_in')) and not request.path.startswith(reverse('clients:sign_up')) and not request.path.startswith(reverse('clients:signup_check_mobile')) and not request.path.startswith(reverse('clients:signup_check_email')):                

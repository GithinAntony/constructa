...
def sign_in(request):
    if request.method == 'POST':
            ...
            if user_registration.objects.filter(mobile_number=data_record['mobile_number'],password=data_record['password'],user_type='civil_engineers'):
                user_details = user_registration.objects.get(mobile_number=data_record['mobile_number'],password=data_record['password'],user_type='civil_engineers')
                if user_details.status == 'active':
                    request.session['is_logged_in'] = True
                    request.session['user_type'] = 'civil_engineers'                    
                    ...
                    return redirect(reverse('civil_engineers:dashboard'))
                else:
                ...
                return redirect(reverse('civil_engineers:sign_in'))
    return render(request, 'civil_engineers/sign-in.html')

def sign_up(request):
    if request.method == 'POST':
        data_record = request.POST
        if user_registration.objects.filter(email_address=data_record['email_address'],user_type='civil_engineers'):
            messages.error(request, 'Email already exists! Please try again!')
            return redirect(reverse('civil_engineers:sign_up'))
        elif user_registration.objects.filter(mobile_number=data_record['mobile_number'],user_type='civil_engineers'):
            messages.error(request, 'Mobile number already exists! Please try again!')
            return redirect(reverse('civil_engineers:sign_up'))
        else:
            profile_photo = request.FILES['profile_photo']
            folder = 'civil_engineers' # subfolder name
            ...
            signup.save()
            messages.success(request, 'Civil engineers registered successfully!')
            return redirect(reverse('civil_engineers:sign_in'))
    context = {}
    return render(request, 'civil_engineers/sign-up.html', context)
...
def dashboard(request):
    ...
    return render(request, 'civil_engineers/dashboard.html', context)   
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(email_address=data_record['email_address'],user_type='civil_engineers').exclude(unique_id=request.session['user_id']):
            messages.error(request, 'Email already exists! Please try again!')
            return redirect(reverse('civil_engineers:edit_profile'))
        elif user_registration.objects.filter(mobile_number=data_record['mobile_number'],user_type='civil_engineers').exclude(unique_id=request.session['user_id']):
            messages.error(request, 'Mobile number already exists! Please try again!')
            return redirect(reverse('civil_engineers:edit_profile'))
        else:
            ...
            return redirect(reverse('civil_engineers:edit_profile')) # redirect to the user detail pagee    
    context = { 'userdata': userdata }
    return render(request, 'civil_engineers/edit-profile.html', context)
...
def business_profile(request):
    ...
    return render(request, 'civil_engineers/business-profile.html', context)
...
def business_profile_edit(request):
    ...
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(business_title=data_record['business_title']).exclude(unique_id=request.session['user_id']).exists():
            ...
            return redirect(reverse('civil_engineers:business_profile_edit'))
        else:
    ...
    return render(request, 'civil_engineers/business-profile-edit.html', context)
...
def signup_check_email(request):
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(email_address=data_record['email_a'],user_type='civil_engineers').exists():
...
def signup_check_mobile(request):
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(mobile_number=data_record['phone_number'],user_type='civil_engineers').exists():
            response_data = {'exists': True}
        else:
            response_data = {'exists': False}
        # Return the result as a JSON response

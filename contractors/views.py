...
def sign_in(request):
    if request.method == 'POST':
            ...
            if user_registration.objects.filter(mobile_number=data_record['mobile_number'],password=data_record['password'],user_type='contractors'):
                user_details = user_registration.objects.get(mobile_number=data_record['mobile_number'],password=data_record['password'],user_type='contractors')
                if user_details.status == 'active':
                    request.session['is_logged_in'] = True
                    request.session['user_type'] = 'contractors'                    
                    ...
                    return redirect(reverse('contractors:dashboard'))
                else:
                    messages.error(request, 'User is suspended. Contact the admin!')
                    return redirect(reverse('contractors:sign_in'))
            else:
                messages.error(request, 'Invalid credentials!')
                return redirect(reverse('contractors:sign_in'))
    return render(request, 'contractors/sign-in.html')

def sign_up(request):
    if request.method == 'POST':
        data_record = request.POST
        if user_registration.objects.filter(email_address=data_record['email_address'],user_type='contractors'):
            messages.error(request, 'Email already exists! Please try again!')
            return redirect(reverse('contractors:sign_up'))
        elif user_registration.objects.filter(mobile_number=data_record['mobile_number'],user_type='contractors'):
            messages.error(request, 'Mobile number already exists! Please try again!')
            return redirect(reverse('contractors:sign_up'))
        else:
            profile_photo = request.FILES['profile_photo']
            folder = 'contractors' # subfolder name
            ...
            return redirect(reverse('contractors:sign_in'))
    context = {}
    return render(request, 'contractors/sign-up.html', context)
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(email_address=data_record['email_address'],user_type='contractors').exclude(unique_id=request.session['user_id']):
            messages.error(request, 'Email already exists! Please try again!')
            return redirect(reverse('contractors:edit_profile'))
        elif user_registration.objects.filter(mobile_number=data_record['mobile_number'],user_type='contractors').exclude(unique_id=request.session['user_id']):
            messages.error(request, 'Mobile number already exists! Please try again!')
            return redirect(reverse('contractors:edit_profile'))
        else:
            ...
            return redirect(reverse('contractors:edit_profile')) # redirect to the user detail pagee    
    context = { 'userdata': userdata }
    return render(request, 'contractors/edit-profile.html', context)
...
def business_profile(request):
    ...
    return render(request, 'contractors/business-profile.html', context)
...
def business_profile_edit(request):
    ...
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(business_title=data_record['business_title']).exclude(unique_id=request.session['user_id']).exists():
            ...
            return redirect(reverse('contractors:business_profile_edit'))
        else:
    ...
    return render(request, 'contractors/business-profile-edit.html', context)
...
def projects_add(request):
    if request.method == 'POST':
        ...
        if projects.objects.filter(title=data_record['title']):
            messages.error(request, 'Title already exists! Please try again!')
            return redirect(reverse('contractors:projects_add'))
        else:
            ...
            return redirect(reverse('contractors:projects_edit', args=[last_inserted_id]))
...
def projects_edit(request,project_id):
    ...
    if request.method == 'POST':
        ...
        if projects.objects.filter(title=data_record['title']).exclude(unique_id=project_id):
            messages.error(request, 'Title already exists! Please try again!')
            return redirect(reverse('contractors:projects_edit', args=[project_id]))
        else:
...
def projects_delete(request,project_id):    
    ...
    return redirect(reverse('contractors:projects_list'))  
...
def signup_check_email(request):
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(email_address=data_record['email_a'],user_type='contractors').exists():
...
def signup_check_mobile(request):
    if request.method == 'POST':
        ...
        if user_registration.objects.filter(mobile_number=data_record['phone_number'],user_type='contractors').exists():
            response_data = {'exists': True}
        else:
            response_data = {'exists': False}
        # Return the result as a JSON response
